import React, { useState, useContext, useEffect } from 'react';
import Button from '@material-ui/core/Button';
import Container from '@material-ui/core/Container';
import MediaQuery from 'react-responsive';
import Grid from '@material-ui/core/Grid';
import TextField from '@material-ui/core/TextField';
import Checkbox from '@material-ui/core/Checkbox';
import { makeStyles } from '@material-ui/core/styles';
import { Link } from 'react-router-dom';
import { signInWithGoogle } from '../../firebase';
import { UserContext } from '../../providers/UserProvider';
import firebase from 'firebase';
import { useHistory } from 'react-router-dom';
import { GoogleSpreadsheet } from 'google-spreadsheet';

const useStyles = makeStyles((theme) => ({
  container: {
    position: 'static',
    marginTop: '1%',
    padding: theme.spacing(3),
  },
}));

const MedSignUpForm = () => {
  const classes = useStyles();

  let history = useHistory();

  const userContext = useContext(UserContext);

  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [findOut, setFindOut] = useState('');
  const [weeklyEmails, setWeeklyEmails] = useState(false);
  const [error, setError] = useState(null);

  const SPREADSHEET_ID = '14oCOE7FBKM8mNSCZ0buBMndhh8ic7ZUONcFQwDX6i2M';
  const SHEET_ID = '0';
  const CLIENT_EMAIL = 'googlesheets@mindful-minutes-4c22c.iam.gserviceaccount.com';
  const PRIVATE_KEY =
    '-----BEGIN PRIVATE KEY-----\\nslDSrTEEOqZwDTZbChYLe+9C\n-----END PRIVATE KEY-----\n';

  const doc = new GoogleSpreadsheet(SPREADSHEET_ID);

  const appendSpreadsheet = async (row) => {
    try {
      await doc.useServiceAccountAuth({
        client_email: CLIENT_EMAIL,
        private_key: PRIVATE_KEY,
      });
      // loads document properties and worksheets
      await doc.loadInfo();

      const sheet = doc.sheetsById[SHEET_ID];
      const result = await sheet.addRow(row);
    } catch (e) {
      console.error('Error: ', e);
    }
  };

  const createUser = (event, name, email, findOut, weeklyEmails) => {
    event.preventDefault();
    const db = firebase.firestore();
    const userRef = db.collection('meditationmans').add({
      name,
      email,
      findOut,
      weeklyEmails,
    });
    userContext.name = name;
    const newRow = { Name: name, Email: email, FindOut: findOut, WeeklyEmails: weeklyEmails };
    appendSpreadsheet(newRow);
    setName('');
    setEmail('');
    setFindOut('');
    setWeeklyEmails(false);
    history.replace('/thankyou');
  };

  const onChangeHandler = (event) => {
    const { name, value } = event.currentTarget;
    if (name === 'email') {
      setEmail(value);
    } else if (name === 'name') {
      setName(value);
    } else if (name === 'findOut') {
      setFindOut(value);
    }
  };
  const clear = () => {
    setName('');
    setEmail('');
    setFindOut('');
    setWeeklyEmails(false);
  };

  const weeklyEmailsChange = (event) => {
    setWeeklyEmails(event.target.checked);
  };

  useEffect(() => {
    if (userContext.user) {
      console.log(userContext.user.displayName);
      setName(userContext.user.displayName);
      setEmail(userContext.user.email);
    }
  }, [userContext.user]);

  return (
    <Container className={classes.container}>
      <form>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <TextField fullWidth value={name} label="Name" name="name" size="small" variant="outlined" required onChange={(event) => onChangeHandler(event)} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth value={email} label="Email" name="email" size="small" variant="outlined" required onChange={(event) => onChangeHandler(event)} />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  value={findOut}
                  label="Where did you find out about us?"
                  name="findOut"
                  size="small"
                  variant="outlined"
                  required
                  onChange={(event) => onChangeHandler(event)}
                />
              </Grid>
              <MediaQuery minDeviceWidth={1024}>
                <Grid item xs={1} style={{ marginTop: '-1%' }}>
                  <Checkbox color="primary" checked={weeklyEmails} onChange={weeklyEmailsChange} inputProps={{ 'aria-label': 'primary checkbox' }} required />
                </Grid>
                <Grid item xs={11} style={{ marginTop: '-1%' }}>
                  <p style={{ fontFamily: 'DMSans-Bold', paddingTop: '10px' }}>Receive weekly emails to access our meditation sessions.</p>
                </Grid>
              </MediaQuery>
              <MediaQuery maxDeviceWidth={1023.9}>
                <Grid item xs={2} style={{ marginTop: '-1%' }}>
                  <Checkbox color="primary" checked={weeklyEmails} onChange={weeklyEmailsChange} inputProps={{ 'aria-label': 'primary checkbox' }} required />
                </Grid>
                <Grid item xs={10} style={{ marginTop: '-1%' }}>
                  <p style={{ fontFamily: 'DMSans-Bold', paddingTop: '10px' }}>Receive weekly emails to access our meditation sessions.</p>
                </Grid>
              </MediaQuery>

              {/* <Grid item xs={9} style={{ marginTop: '-10px' }}></Grid> */}
              {/* <Grid item xs={3} style={{ marginTop: '-10px' }}>
                <Button
                  size="small"
                  style={{ background: '#2C354A' }}
                  variant="contained"
                  onClick={() => {
                    clear();
                  }}
                >
                  <p
                    style={{
                      color: 'white',
                      marginBottom: '0px',
                      marginTop: '0px',
                    }}
                  >
                    Clear
                  </p>
                </Button>
              </Grid> */}
            </Grid>
          </Grid>
          <Grid item xs={9}></Grid>
          <Grid item xs={3} style={{ marginTop: '-2%' }}>
            <MediaQuery minDeviceWidth={1024}>
              <Button
                size="large"
                style={{ background: '#2C354A' }}
                fullWidth
                type="submit"
                variant="contained"
                onClick={(event) => {
                  createUser(event, name, email, findOut, weeklyEmails);
                }}
              >
                <h6
                  style={{
                    color: 'white',
                    paddingTop: '3px',
                    marginBottom: '0px',
                    fontFamily: 'DMSans-Bold',
                    fontSize: '1.1vw',
                  }}
                >
                  Sign Up
                </h6>
              </Button>
            </MediaQuery>
            <MediaQuery maxDeviceWidth={1023.9}>
              <Button
                size="large"
                style={{ background: '#2C354A' }}
                fullWidth
                type="submit"
                variant="contained"
                onClick={(event) => {
                  createUser(event, name, email, findOut, weeklyEmails);
                }}
              >
                <h6
                  style={{
                    color: 'white',
                    paddingTop: '3px',
                    marginBottom: '0px',
                    fontFamily: 'DMSans-Bold',
                    fontSize: '2.5vw',
                  }}
                >
                  Sign Up
                </h6>
              </Button>
            </MediaQuery>
          </Grid>
        </Grid>
      </form>
      {/* <p className="text-center my-3">or</p>
      <Button
        style={{ background: '#2C354A', marginLeft: 'auto', marginRight: 'auto' }}
        variant="contained"
        onClick={signInWithGoogle}
      >
        <h6
          style={{
            color: 'white',
            paddingTop: '1px',
            marginBottom: '0px',
          }}
        >
          Sign Up with Google
        </h6>
      </Button> */}
    </Container>
  );
};

export default MedSignUpForm;
